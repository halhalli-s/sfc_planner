...
int main(int argc, char **argv)
{
    ...
    map_params.resolution = 0.5; // Increased from 0.05 to 0.5 for faster A*
    ...
    admm_params.inner_max = 5;
    admm_params.rho = 1.0;
    admm_params.epsilon = 0.3;
    admm_params.bbox_size = 2.0;
    admm_params.wc = 1.0;
    admm_params.wv = 1.0;
    ...
    {
        const auto &wp = sfc.get_waypoints()[i];
        std::cout << "  " << i << ": [" << wp.x() << ", " << wp.y() << ", " << wp.z() << "]" << std::endl;
    }
    ...
    {
        const auto &ell = sfc.get_ellipsoids()[i];
